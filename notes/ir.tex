...
The \code{Equality} constraint implements the \code{Value} trait. The constraint's value
is computed by applying the \code{method} on the \code{left} and \code{right} members.
Given that \code{method} must be a method of the \code{Equality} trait, the constraint's
value is a Boolean. The \code{set} method has no effect.

Both the \code{left} and \code{right} members MUST be specified (as a constraint described
in this section), and MUST have values that implement the \code{Equality} trait.
...
The \code{Ordinal} constraint implements the \code{Value} trait. The constraint's value is
computed by applying the \code{method} on the \code{left} and \code{right} members. Given
that \code{method} must be a method of the \code{Ordinal} trait, the constraint's value is
a Boolean. The \code{set} method has no effect.

Both the \code{left} and \code{right} members MUST be specified (as a constraint described
in this section), and MUST have values that implement the \code{Ordinal} trait.
...
The \code{Boolean} constraint implements the \code{Value} trait. The constraint's value is
computed by applying the \code{method} on the \code{left} and \code{right} members. Given
that \code{method} must be a method of the \code{BooleanOps} trait, the constraint's value
is a Boolean. The \code{set} method has no effect.

Both the \code{left} and \code{right} members MUST be specified (as a constraint described
in this section), and MUST have values that implement the \code{BooleanOps} trait.
...
The \code{Arithmetic} constraint implements the \code{Value} trait. The constraint's value
is computed by applying the \code{method} on the \code{left} and \code{right} members. The
\code{set} method has no effect.

Both the \code{left} and \code{right} members MUST be specified (as a constraint described
in this section), and MUST have values that implement the \code{ArithmeticOps} trait.
...
The \code{condition} MUST be a constraint whose value is a Boolean.

The \code{Conditional} constraint implements the \code{Value} trait. If the
\code{condition} evaluates to \code{True}, then the constraint's value is the \code{true}
member. Otherwise, the constraint's value is the \code{false} member. The \code{set}
...
The \code{Constant} constraint implements the \code{Value} trait. The constraint's value
is specified in the \code{value} member.
...
\begin{alltt}
  \{
    "constraint"   : "Field",
    "field"        : \emph{Value getter} or
                     \emph{NamedCollection getter} or
                     \emph{NumberedCollection getter}
  \}
\end{alltt}
\normalsize

The JSON form of a \code{Value} getter is:
...
A \code{Value} getter is used when the \code{name} member refers to a field that
implements the \code{Value} trait. The \code{Value} getter takes on the value of the field
specified in \code{name}.
...
\begin{alltt}
  \{
    "name"         : "\emph{Name of field that implements
                      NamedCollection trait}",
    "member"       : \emph{Value getter} or
                     \emph{NamedCollection getter} or
                     \emph{NumberedCollection getter}
  \}
\end{alltt}
...
A \code{NamedCollection} getter is used when the \code{name} member refers to a field that
implements the \code{NamedCollection} trait. The \code{NamedCollection} getter takes on
the value of the \code{member} member.

The JSON form of a \code{NumberedCollection} getter is:

\footnotesize
\begin{alltt}
  \{
    "name"         : "\emph{Name of field that implements
                      NumberedCollection trait}",
    "index"        : \emph{index},
    "member"       : \emph{Value getter} or
                     \emph{NamedCollection getter} or
                     \emph{NumberedCollection getter} or
                     None
  \}
\end{alltt}
\normalsize

A \code{NumberedCollection} getter is used when the \code{name} member refers to a field
that implements the \code{NumberedCollection} trait.
