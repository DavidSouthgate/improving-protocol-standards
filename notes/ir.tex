...
Representable types MUST implement the \code{Size} trait.
...
The \code{FieldName} type denotes the name of a field in a \code{NamedCollection}. 
It implements the traits \code{Value} and \code{Equality}.
...
The runtime defines the \code{Size} trait. This defines a method
...
be used to get and set the values of fields of a map-like data structure, which has
...
Instances of bit string types implement the \code{Size} trait. The size
MAY be unspecified.
...
Arrays implement the \code{Size} trait. The size of the array is \code{length}
...
A structure type implements the \code{Size} trait. The size of a structure is the
sum of the sizes of its fields.

A structure type implements the \code{NamedCollection} trait. Its elements,
\code{fields}, are accessed by \code{name}. A structure's \code{length} is the number
of fields that it contains.
...
An enumerated type implements the \code{Size} trait. The size of an enumerated type
...
A function type implements the \code{Size} trait. The size of the function is that of the
\code{return\_type}.
...
\end{alltt}
\normalsize
When invoked on a structure type, returns a reference to the specified
field of the structure. If invoked on the parsing context object, this
returns a reference to the specified field in the parsing context. It
an error to invoke \code{FieldAccess} on something other than a structure
type or the parsing context.
