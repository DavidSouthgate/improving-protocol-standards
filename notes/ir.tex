...
multi-bit values that can be parsed or serialised. The constructed type
...
and 
leaf (\code{Constant} and \code{TypeInstance}).
...
on \code{self}, with the arguments specified in \code{arguments}.
...
A \code{TypeInstance} leaf expression is represented in the JSON form of the
intermediate representation as:
\footnotesize
\begin{alltt}
  \{
    "expression"   : "TypeInstance",
    "name"         : "\emph{type instance name} or
                      self"
  \}
\end{alltt}
\normalsize
The \code{TypeInstance} expression evaluates to the \code{name} member. \code{name} MUST
be a name in the global type namespace, or ``self''. If \code{name} is ``self'', the 
\code{TypeInstance} expression evaluates to the containing type instance.
