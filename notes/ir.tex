...
Each field has a \emph{constraint}, \code{is\_present}, that indicates
...
Both the per-field \code{is\_present} constraint, and struct-level
\code{constraints} on members, MUST evaluate to \code{Boolean} values.
...
\end{alltt}
\normalsize
At most one context can be instantiated. If a context is instantiated, it
is named \code{\$Context} in the global type namespace. 
...
Constraints are split into two classes: 
tree (\code{Equality}, \code{Ordinal}, \code{Boolean}, \code{Arithmetic}, and \code{IfElse})
and 
leaf (\code{Constant} and \code{Field}).

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Tree Constraints}

An \code{Equality} tree constraint is represented in the JSON form of the
...
An \code{Ordinal} tree constraint is represented in the JSON form of the
intermediate representation as:
\footnotesize
\begin{alltt}
  \{
...
A \code{Boolean} tree constraint is represented in the JSON form of the
intermediate representation as:
\footnotesize
\begin{alltt}
  \{
...
An \code{Arithmetic} tree constraint is represented in the JSON form of the
intermediate representation as:
\footnotesize
\begin{alltt}
  \{
...
A \code{IfElse} tree constraint is represented in the JSON form of the
intermediate representation as:
\footnotesize
\begin{alltt}
  \{
    ...
    "if\_true"     : \emph{constraint},
    "if\_false"    : \emph{constraint}
  \}
\end{alltt}
\normalsize
The \code{IfElse} includes a \code{condition} that evaluates to the \code{Boolean} type.
The \code{IfElse} constraint evaluates to the \code{if\_true} member, if \code{condition}
is \code{True}. Otherwise, the constraint evaluates to the \code{if\_false} member.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Leaf Constraints}

A \code{Constant} leaf constraint is represented in the JSON form of the
intermediate representation as:
\footnotesize
\begin{alltt}
  \{
...
A \code{Field} leaf constraint is represented in the JSON form of the
intermediate representation as:
\footnotesize
\begin{alltt}
  \{
...
For example, a leaf constraint referring to the field \code{seq\_num} of a
...
\end{alltt}
\normalsize
A leaf constraint referring to the 4th element of an array field named
...
\end{alltt}
\normalsize
A leaf constraint referring to the \code{length} attribute of an array
