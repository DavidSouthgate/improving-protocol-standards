...
tree (\code{MethodInvocation}, \code{FunctionInvocation}, and \code{IfElse})
and 
leaf (\code{Constant}).
...
\begin{alltt}
  \{
    ...
    "method"       : "\emph{name of any method implemented}
                      \emph{by} self \emph{type}",
    "self"         : "\emph{this}" or
                     "\emph{context}" or
    				 \emph{expression},
    "arguments"    : [
...
on \code{self}, with the arguments specified in \code{arguments}. If \code{self} is
``this'' or ``context'', \code{self} refers to either the containing structure, or the
context (if it exists), respectively.
...
An expression referring to the length attribute of an array field named \code{csrc\_list}
in a structure would be written:
\footnotesize
\begin{alltt}
  \{
    "expression"   : "MethodInvocation",
