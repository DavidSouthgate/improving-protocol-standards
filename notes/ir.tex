...
Each field has an \code{transform} \code{FunctionInvocation} expression. The
\code{transform} field MAY be \code{null}. 
...
Finally, on the successful parsing of the struct, the expressions
in \code{actions} are evaluated. The set of actions MAY be empty. The expressions
in the set of actions MUST be \code{Mutator} tree expressions.
...
A \code{Mutator} tree expression is represented in the JSON form of the
intermediate representation as:
\footnotesize
\begin{alltt}
  \{
...
\end{alltt}
\normalsize
The \code{Mutator} tree expression sets the field specified in \code{left} to the value
expressed in \code{right}. The \code{Mutator} tree expression itself evaluates to
\code{Nothing}.
...
\end{alltt}
\normalsize
\code{name} MUST refer to a previously defined \code{Function}. The set of
\code{arguments} MUST contain values for all of the parameters specified in the
\code{Function} definition. The \code{value} expressions MUST evaluate to the parameter
type specified in the \code{Function} definition. The \code{FunctionInvocation} expression
itself evaluates to the \code{return\_type} specified in the \code{Function} definition.
