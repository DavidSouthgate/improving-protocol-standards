...
Limits on how many of each type can be instantiated

How do the IR and runtime relate?

%--------------------------------------------------------------------------------------------------
\subsection{Input parsers \& output formatters}
\label{sec:gpl-formats}

Need to be clear that input and output formats are arbitrary.
...
LongHeader := {
	...
	packet_num = lh_packetnum_decrypt(ppacket_num);
	Context.dcid = scid;
};
    \end{BVerbatim}
...
In this section, we will demonstrate that the novel features of our abstract
representation language -- context and helper functions -- are necessary. We begin by
describing QUIC's long (Figure~\ref{fig:quic-long-hdr-desc}) and short
(Figure~\ref{fig:quic-short-hdr-desc}) header formats using a simple protocol description
language. As discussed in Section \ref{sec:formalising}, this language is used to generate
the abstract protocol representation. The syntax of this language is largely unimportant -
a key characteristic of the Glasgow Packet Language is that it can be generated from an
arbitrary protocol description. We also note that this example is not exhaustive: our goal
is demonstrate that the GPL \emph{can} be used to model the parsing of QUIC; we will give
examples that can be expanded to other protocol features.
...
and applies the decryption algorithm, as discussed the QUIC TLS draft (Section 5.3,
\cite{draft-ietf-quic-tls-14}). At present, it is not possible to see this relationship by
looking at the ASCII format alone: the prose description must be read, and the definition
cross-referenced with another document.

In addition, once a long header format PDU is parsed, the incoming Source Connection ID is stored.
As seen in the description of the short header format
(Figure~\ref{fig:quic-short-hdr-desc}), the Source Connection ID received in the long
header becomes the source 
